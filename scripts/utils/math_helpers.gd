...
static func get_hp_missing_bonus(current_hp: int, max_hp: int, max_bonus: float = Constants.CAPTURE_HP_BONUS_MAX) -> float:
...
static func calculate_defense_reduction(defense: int, factor: float = Constants.DAMAGE_DEFENSE_FACTOR) -> float:
