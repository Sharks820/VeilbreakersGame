...
static func log_ui_character_action(action: String, character_name: String, is_protagonist: bool = false) -> void:
	if DEBUG_ENABLED and LOG_BATTLE_UI:
		print("[BATTLE_UI] %s - character: %s, is_protagonist: %s" % [action, character_name, is_protagonist])

## Log battle UI setup
static func log_ui_setup(player_count: int, enemy_count: int) -> void:
	if DEBUG_ENABLED and LOG_BATTLE_UI:
		print("[BATTLE_UI] setup_battle called with %d players, %d enemies" % [player_count, enemy_count])

## Log waiting for input
static func log_ui_waiting(character_name: String, is_protagonist: bool) -> void:
	if DEBUG_ENABLED and LOG_BATTLE_UI:
		print("[BATTLE_UI] Waiting for input - character: %s, is_protagonist: %s" % [character_name, is_protagonist])
...
static func log_combat_damage(target_name: String, damage: int, is_critical: bool) -> void:
	if DEBUG_ENABLED and LOG_COMBAT_LOG:
		print("[COMBAT_LOG] DAMAGE: %s took %d damage (crit: %s)" % [target_name, damage, is_critical])
...
static func log_combat_result(character_name: String, action: int, result_keys: Array) -> void:
	if DEBUG_ENABLED and LOG_COMBAT_LOG:
		print("[COMBAT_LOG] Action executed - character: %s, action: %d, result keys: %s" % [character_name, action, result_keys])
...
## Log capture success
static func log_capture_success(monster_name: String, method_name: String, capture_chance: float) -> void:
...
## Log capture failure
static func log_capture_failure(monster_name: String, method_name: String, capture_chance: float) -> void:
	if DEBUG_ENABLED and LOG_BATTLE_MANAGER:
		EventBus.emit_debug("%s escaped capture via %s (chance: %.0f%%)" % [monster_name, method_name, capture_chance * 100])
...
## Log animation state
static func log_animation(sprite_name: String, animation_type: String, context: String = "") -> void:
	if DEBUG_ENABLED and LOG_ANIMATION:
		if context.is_empty():
			print("[BattleArena] Playing %s animation for %s" % [animation_type, sprite_name])
		else:
			print("[BattleArena] %s - sprite: %s, context: %s" % [animation_type, sprite_name, context])
