...
static func emit_quest_updated(local_signal: Signal, quest_id: String, objective_index: int) -> void:
	...
	for item in signals_and_args:
		if item.size() >= 1 and item[0] is Signal:
			...
			match args.size():
				...
				_: push_warning("SignalHelpers.emit_batch: Too many arguments")
