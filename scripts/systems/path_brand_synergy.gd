...
static func calculate_party_synergy(hero: Node, party_monsters: Array) -> Dictionary:
	...
	var result := {"tier": 0, "synergy_count": 0, "bonuses": {}, "synergy_monsters": []}
...
static func _add_alignment_bonus(
	bonuses: Dictionary, hero: Node, synergy_brands: Array
) -> Dictionary:
	...
	for brand_key in synergy_brands:
		if (
			brand_key
			in ["BLOODIRON", "CORROSIVE", "VENOMSTRIKE", "TERRORFLUX", "NIGHTLEECH", "RAVENOUS"]
		):
