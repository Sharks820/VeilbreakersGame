...
func activate_shrine(shrine_id: String, tier: Enums.ShrineTier) -> Dictionary:
	...
	var result := {"success": false, "reason": "", "available_actions": []}
...
func rest() -> Dictionary:
	...
	var result := {"hp_restored": 0, "sp_restored": 0, "debuffs_cleared": 0}
...
func purify_monster(monster: Monster) -> Dictionary:
	...
	if sanctum_energy < Constants.SHRINE_CLEANSE_ENERGY_COST:
		result.reason = (
			"Not enough Sanctum Energy (%d required)" % int(Constants.SHRINE_CLEANSE_ENERGY_COST)
		)
		return result
...
func commune() -> Dictionary:
	...
	var result := {"dialogue_triggered": false, "hint_given": false, "glitch_occurred": false}
