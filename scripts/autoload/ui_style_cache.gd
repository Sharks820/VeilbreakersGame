...
func _create_predefined_styles() -> void:
	...
	# Panel backgrounds
	panel_dark = _create_flat(Color(0.1, 0.1, 0.15, 0.85), Color(0.3, 0.25, 0.4, 1.0), 2, 8)

	panel_dark_red = _create_flat(Color(0.15, 0.1, 0.1, 0.85), Color(0.5, 0.25, 0.25, 1.0), 2, 8)

	panel_dark_blue = _create_flat(Color(0.1, 0.1, 0.18, 0.85), Color(0.25, 0.3, 0.5, 1.0), 2, 8)

	panel_transparent = _create_flat(Color(0, 0, 0, 0), Color(0, 0, 0, 0), 0, 0)

	# HP bars
	hp_fill_ally = _create_flat(Color(0.2, 0.8, 0.3, 1.0), Color(0, 0, 0, 0), 0, 2)

	hp_fill_enemy = _create_flat(Color(0.8, 0.2, 0.2, 1.0), Color(0, 0, 0, 0), 0, 2)

	hp_bg = _create_flat(Color(0.15, 0.1, 0.1, 0.9), Color(0, 0, 0, 0), 0, 2)

	# MP bars
	mp_fill = _create_flat(Color(0.2, 0.4, 0.9, 1.0), Color(0, 0, 0, 0), 0, 2)

	mp_bg = _create_flat(Color(0.1, 0.1, 0.15, 0.9), Color(0, 0, 0, 0), 0, 2)

	# Corruption bars
	corruption_fill = _create_flat(Color(0.5, 0.1, 0.6, 1.0), Color(0, 0, 0, 0), 0, 2)

	corruption_bg = _create_flat(Color(0.1, 0.05, 0.1, 0.9), Color(0, 0, 0, 0), 0, 2)

	# Button styles
	btn_normal = _create_flat(Color(0.12, 0.12, 0.15, 0.95), Color(0.4, 0.35, 0.3, 0.8), 2, 6)

	btn_hover = _create_flat(Color(0.18, 0.18, 0.22, 0.98), Color(0.6, 0.5, 0.4, 1.0), 2, 6)

	btn_pressed = _create_flat(Color(0.08, 0.08, 0.1, 0.98), Color(0.5, 0.4, 0.3, 1.0), 2, 6)

	btn_disabled = _create_flat(Color(0.08, 0.08, 0.1, 0.7), Color(0.3, 0.3, 0.3, 0.5), 2, 6)

	btn_focus = _create_flat(Color(0.18, 0.14, 0.22, 0.95), Color(0.7, 0.55, 0.4, 1.0), 2, 6)

	# Tooltip
	tooltip_bg = _create_flat(Color(0.08, 0.08, 0.12, 0.95), Color(0.4, 0.35, 0.5, 1.0), 1, 4)

	# Portrait frames
	portrait_frame_ally = _create_flat(
		Color(0.15, 0.15, 0.2, 1.0), Color(0.4, 0.35, 0.5, 1.0), 2, 4
	)

	portrait_frame_enemy = _create_flat(Color(0.2, 0.1, 0.1, 1.0), Color(0.6, 0.3, 0.3, 1.0), 2, 4)
...
func _create_flat(
	bg_color: Color, border_color: Color, border_width: int, corner_radius: int
) -> StyleBoxFlat:
...
func _make_cache_key(
	bg_color: Color, border_color: Color, border_width: int, corner_radius: int, content_margin: int
) -> String:
	## Create a unique cache key from style properties
	return (
		"%s_%s_%d_%d_%d"
		% [bg_color.to_html(), border_color.to_html(), border_width, corner_radius, content_margin]
	)
...
func print_cache_stats() -> void:
	...
	print(
		(
			"[UIStyleCache] Hits: %d, Misses: %d, Cached: %d, Hit Rate: %.1f%%"
