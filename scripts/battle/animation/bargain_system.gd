...
# ENUMS
# -----------------------------------------------------------------------------
enum Approach { DOMINATE, PROMISE, BREAK }  # Force submission  # Offer a vow  # Exploit weakness

enum ResponseType { ACCEPT, REJECT, WAVER, RAGE }  # Creature accepts the binding  # Creature rejects and counterattacks  # Creature hesitates (partial success?)  # Creature is deeply offended (worse counterattack)
...
func _handle_failure(approach: Approach, roll: float) -> void:
	...
	match approach:
		...
		Approach.PROMISE:
			...
			failure_dialogue = (
				BRAND_PROMISES.get(creature_brand, BRAND_PROMISES["SAVAGE"]).rejection
			)
...
# =============================================================================
# INTEGRATION WITH BATTLE SEQUENCER
# =============================================================================
#
# Replace the old capture sequence in BattleSequencer with:
#
# func _execute_capture(source: Node, target: Node, action: Dictionary) -> void:
#     battle_stats.captures_attempted += 1
#
#     # Initiate the bargain
#     var bargain = $BargainSystem  # or get_node reference
#
#     bargain.bargain_succeeded.connect(_on_bargain_success, CONNECT_ONE_SHOT)
#     bargain.bargain_failed.connect(_on_bargain_failure, CONNECT_ONE_SHOT)
#
#     bargain.initiate_bargain(target, source)
#
#     # Wait for player choice (UI shows options)
#     # UI calls bargain.make_choice(approach) when player decides
#
#     await bargain.bargain_ended
#
# func _on_bargain_success(creature: Node, approach: String, bonus: Dictionary) -> void:
#     battle_stats.captures_successful += 1
#
#     # Apply purification
#     if creature.has_method("purify"):
#         creature.purify(bonus.purification_amount)
#
#     # Apply bonus stats
#     for stat in bonus.bonus_stats:
#         if creature.has_method("add_bonus"):
#             creature.add_bonus(stat, bonus.bonus_stats[stat])
#
#     # Remove from battle, add to party
#     enemy_party.erase(creature)
#     capture_result.emit(creature, true)
#
# func _on_bargain_failure(creature: Node, approach: String, counterattack: Dictionary) -> void:
#     # Creature counterattacks
#     var damage = int(current_actor.get_max_hp() * counterattack.damage_percent)
#     current_actor.take_damage(damage, creature.get_brand(), creature)
#
#     # Apply rage buff if applicable
#     if counterattack.creature_gets_buff:
#         creature.apply_buff(counterattack.buff_type, 1.5, counterattack.buff_duration)
#
#     capture_result.emit(creature, false)
