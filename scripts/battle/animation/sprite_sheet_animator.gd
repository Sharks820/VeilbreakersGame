	...
	for i in range(_sheet_configs.size()):
		...
		if not _loaded_sheets.has(sheet_config.sheet_path):
			...
			if texture:
				...
				push_warning(
					"SpriteSheetAnimator: Failed to load sheet: %s" % sheet_config.sheet_path
				)
...
func play(anim_name: String, reversed: bool = false) -> void:
	...
	if not _animations.has(anim_name):
		...
		if fallback == "":
			push_warning(
				"SpriteSheetAnimator: Unknown animation '%s' for %s" % [anim_name, _monster_id]
			)
			return
		anim_name = fallback
	...
	if _frame_timer >= frame_duration:
		...
		if anim.events.has(_current_frame):
			animation_event.emit(
				anim.events[_current_frame],
				{"animation": _current_animation, "frame": _current_frame}
			)
...
func _process_breathing(delta: float) -> void:
	...
	# AAA QUALITY: Use smoothstep for organic breathing feel
	var breath_raw := sin(_breathing_time * TAU)
	var breath_normalized := (breath_raw + 1.0) * 0.5  # 0 to 1
	# Apply smoothstep for more natural in/out easing
...
func play_attack(on_hit: Callable = Callable()) -> void:
	...
	if _animations.has("attack"):
		...
		if on_hit.is_valid():
			...
			var connection := func(): on_hit.call()
	...
	if anim_to_play != "":
		...
		if on_cast.is_valid():
			if has_hit_frame:
				...
				var connection := func(): on_cast.call()
				hit_frame_reached.connect(connection, CONNECT_ONE_SHOT)
			else:
				# Trigger at midpoint
...
func play_spawn() -> void:
	...
	tween.tween_property(sprite, "scale", _original_scale, 0.4).set_ease(Tween.EASE_OUT).set_trans(
		Tween.TRANS_BACK
	)
...
func _procedural_hurt(is_critical: bool) -> void:
	...
	tween.tween_property(
		sprite, "position", _original_position + Vector2(-25 * dir * intensity, 0), 0.1
	)
