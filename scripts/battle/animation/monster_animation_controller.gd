...
func _play_animation(anim_name: String, lock: bool = true) -> void:
	...
	if not _has_animation(anim_name):
		push_warning(
			"MonsterAnimationController: Missing animation '%s' for %s" % [anim_name, monster_name]
		)
		return
...
func _request_special_effect() -> void:
	...
	effect_spawn_requested.emit(
		"special_" + brand.to_lower(), {"position": position, "brand": brand}
	)
...
func _flash_white() -> void:
	...
	_flash_tween.tween_property(
		flash_material, "shader_parameter/flash_amount", 0.0, hit_flash_duration
	)
...
func _squash() -> void:
	...
	_squash_tween.tween_property(
		...
		(
			_original_scale
			* Vector2(1.0 - squash_stretch_amount * 0.5, 1.0 + squash_stretch_amount * 0.5)
		),
		0.1
	)
...
func play_death() -> void:
	...
	if dissolve_material:
		...
		(
			tween
			. tween_property(
				dissolve_material, "shader_parameter/dissolve_amount", 1.0, death_sequence_time
			)
...
func play_capture_break() -> void:
	...
	# Burst out effect
	effect_spawn_requested.emit(
		"capture_break_burst", {"position": global_position, "brand": brand}
	)
...
func play_spawn() -> void:
	...
	(
		tween
		. tween_property(animated_sprite, "scale", _original_scale, 0.4)
	...
	effect_spawn_requested.emit(
		"summon_" + brand.to_lower(), {"position": global_position, "brand": brand}
	)
...
func play_phase_transition(phase: int) -> void:
	...
	effect_spawn_requested.emit(
		"phase_transition_" + brand.to_lower(), {"position": global_position, "phase": phase}
	)
