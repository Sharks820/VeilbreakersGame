...
func show_letterbox(duration: float = -1.0) -> void:
	...
	if letterbox_bottom:
		...
		(
			tween_bottom
			. tween_property(
				letterbox_bottom, "position:y", viewport_height - letterbox_height, duration
			)
...
func hide_letterbox(duration: float = -1.0) -> void:
	...
	if letterbox_top:
		var tween_top = create_tween()
		tween_top.tween_property(letterbox_top, "position:y", -letterbox_height, duration).set_ease(
			Tween.EASE_IN
		)
	...
	if letterbox_bottom:
		...
		(
			tween_bottom
			. tween_property(letterbox_bottom, "position:y", viewport_height, duration)
