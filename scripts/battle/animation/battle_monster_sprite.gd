...
const SpriteSheetAnimatorScript := preload(
	"res://scripts/battle/animation/sprite_sheet_animator.gd"
)
		...
		for y in range(32):
			...
			if dist < 1.0:
				...
				shadow_image.set_pixel(
					x, y, Color(shadow_color.r, shadow_color.g, shadow_color.b, alpha)
				)
...
func _start_hollow_shadow_pulse() -> void:
	...
	(
		_shadow_pulse_tween
		. tween_property(shadow_sprite, "scale", Vector2(4.5, 3.0), 1.2)
	...
	(
		_shadow_pulse_tween
		. parallel()
		. tween_property(shadow_sprite, "modulate", Color(1.5, 0.8, 0.8, 1.0), 1.2)
		. set_ease(Tween.EASE_IN_OUT)
		. set_trans(Tween.TRANS_SINE)
	)
	...
	(
		_shadow_pulse_tween
		. tween_property(shadow_sprite, "scale", Vector2(3.0, 2.0), 1.2)
		. set_ease(Tween.EASE_IN_OUT)
		. set_trans(Tween.TRANS_SINE)
	)
	(
		_shadow_pulse_tween
		. parallel()
		. tween_property(shadow_sprite, "modulate", Color(1.0, 0.5, 0.5, 0.8), 1.2)
		. set_ease(Tween.EASE_IN_OUT)
		. set_trans(Tween.TRANS_SINE)
	)
...
func _on_animation_finished(anim_name: String) -> void:
	...
	if (
		anim_name in ["attack", "attack_heavy", "hurt", "hurt_heavy", "spawn"]
