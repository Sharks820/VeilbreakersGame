...
enum ActionType { ATTACK, SKILL, ITEM, CAPTURE, SWAP, DEFEND, FLEE }
...
func _execute_attack(source: Node, targets: Array, action: Dictionary) -> void:
	...
	var is_critical = (
		randf() < (source.get_crit_chance() if source.has_method("get_crit_chance") else 0.1)
	)

	# 1. WINDUP - Attacker prepares
	camera_command.emit(
		"focus_between",
		{"from": source, "to": targets[0], "bias": 0.3, "duration": action_windup_time}
	)
...
func _perform_single_hit(
	...
	camera_command.emit(
		"impact_freeze",
		{"duration": impact_pause_time if not is_critical else impact_pause_time * 2}
	)
	...
	vfx_command.emit(
		"spawn_hit_effect",
		{"brand": brand, "position": target.global_position, "is_critical": is_critical}
	)
	...
	# Audio
	audio_command.emit(
		"play_sfx", {"sound": "hit_" + brand.to_lower(), "position": target.global_position}
	)
	if is_critical:
...
func _execute_skill(source: Node, targets: Array, action: Dictionary) -> void:
	...
	var brand = skill.get(
		"brand", source.get_brand() if source.has_method("get_brand") else "NEUTRAL"
	)
...
func _execute_damage_skill(source: Node, targets: Array, skill: Dictionary, brand: String) -> void:
	...
	for hit in range(hit_count):
		for target in targets:
			var damage = int(
				(source.get_attack() if source.has_method("get_attack") else 10) * base_power
			)
...
func _execute_heal_skill(source: Node, targets: Array, skill: Dictionary) -> void:
	...
	for target in targets:
		var heal_amount = int(
			(source.get_magic() if source.has_method("get_magic") else 10) * heal_power
		)

		vfx_command.emit("heal_effect", {"position": target.global_position})
		vfx_command.emit(
			"spawn_damage_number",
			{"position": target.global_position, "amount": heal_amount, "is_heal": true}
		)
...
func _execute_buff_skill(source: Node, targets: Array, skill: Dictionary) -> void:
	...
	for target in targets:
		vfx_command.emit(
			"buff_effect", {"position": target.global_position, "buff_type": buff_type}
		)
		...
		ui_command.emit(
			"show_status_popup",
			{"target": target, "status": buff_type.to_upper() + " UP", "positive": true}
		)
...
func _execute_debuff_skill(source: Node, targets: Array, skill: Dictionary) -> void:
	...
	for target in targets:
		vfx_command.emit(
			"debuff_effect", {"position": target.global_position, "debuff_type": debuff_type}
		)
		...
		ui_command.emit(
			"show_status_popup",
			{"target": target, "status": debuff_type.to_upper() + " DOWN", "positive": false}
		)
...
func _execute_status_skill(source: Node, targets: Array, skill: Dictionary, brand: String) -> void:
	...
	for target in targets:
		if randf() < chance:
			vfx_command.emit(
				"status_apply",
				{"position": target.global_position, "status": status, "brand": brand}
			)
			...
			ui_command.emit(
				"show_status_popup",
				{"target": target, "status": status.to_upper(), "positive": false}
			)
...
func _execute_capture(source: Node, target: Node, action: Dictionary) -> void:
	...
	var base_chance = 0.3
	var hp_modifier = (
		1.0 - (target.get_hp_percent() if target.has_method("get_hp_percent") else 0.5)
	)
	var status_modifier = (
		0.1 if (target.has_method("has_status") and target.has_status("any")) else 0.0
	)
	...
	for i in range(capture_shake_count):
		...
		vfx_command.emit(
			"capture_shake", {"position": target.global_position, "shake_number": i + 1}
		)
	...
	match item_type:
		"heal":
			for target in targets:
				...
				vfx_command.emit(
					"spawn_damage_number",
					{"position": target.global_position, "amount": amount, "is_heal": true}
				)
				if target.has_method("heal"):
					target.heal(amount)
		"revive":
...
func _apply_poison_tick(entity: Node, status: Dictionary) -> void:
	...
	vfx_command.emit(
		"spawn_damage_number",
		{"position": entity.global_position, "amount": damage, "brand": "VENOM"}
	)
...
func _apply_burn_tick(entity: Node, status: Dictionary) -> void:
	...
	vfx_command.emit(
		"spawn_damage_number",
		{"position": entity.global_position, "amount": damage, "brand": "SURGE"}
	)
...
func _apply_regen_tick(entity: Node, status: Dictionary) -> void:
	...
	vfx_command.emit(
		"spawn_damage_number", {"position": entity.global_position, "amount": heal, "is_heal": true}
	)
...
func _apply_bleed_tick(entity: Node, status: Dictionary) -> void:
	...
	vfx_command.emit(
		"spawn_damage_number",
		{"position": entity.global_position, "amount": damage, "brand": "SAVAGE"}
	)
...
func _trigger_boss_phase_transition(boss: Node, new_phase: int) -> void:
	...
	ui_command.emit(
		"phase_announcement",
		{
			...
			"phase_name":
			(
				boss.get_phase_name(new_phase)
				if boss.has_method("get_phase_name")
				else "Phase " + str(new_phase)
			)
		}
	)
...
func _check_battle_state() -> void:
	...
	var enemies_alive = enemy_party.filter(
		func(e): return e.is_alive() if e.has_method("is_alive") else true
	)
	...
	var players_alive = player_party.filter(
		func(p): return p.is_alive() if p.has_method("is_alive") else true
	)
