	...
	for phase in phases:
		...
		if phase.has("glow"):
			...
			var glow_color := Color(
				1.0 + intensity * 0.2, 1.0 + intensity * 0.2, 1.0 + intensity * 0.15, 1.0
			)
		...
		if phase.has("event"):
			animation_event.emit(
				phase["event"], {"phase": phase.get("name", ""), "animation": anim_name}
			)
