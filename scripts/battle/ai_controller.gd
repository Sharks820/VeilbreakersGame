...
func _acquire_damage_calculator() -> void:
	...
	push_warning(
		"AIController: Could not find parent's DamageCalculator, creating fallback instance"
	)
...
func _default_ai(character: CharacterBase, _allies: Array, enemies: Array) -> Dictionary:
	...
	return {"action": Enums.BattleAction.ATTACK, "target": target, "skill": ""}
...
func _select_optimal_target(
	attacker: CharacterBase, targets: Array[CharacterBase], is_skill: bool
) -> CharacterBase:
...
func _calculate_target_score(
	attacker: CharacterBase, target: CharacterBase, _is_skill: bool
) -> float:
...
func _ai_aggressive(
	...
	_allies: Array,
	_enemies: Array,
	basic_targets: Array[CharacterBase],
	skill_targets: Array[CharacterBase]
) -> Dictionary:
			...
			for skill_id in monster.known_skills:
				if skill_id.contains("heal") or skill_id.contains("restore"):
					if monster.can_use_skill(skill_id):
						return {
							"action": Enums.BattleAction.SKILL, "target": ally, "skill": skill_id
						}
...
func _ai_support(
	...
	if lowest_ally and lowest_percent < 0.5:
		for skill_id in monster.known_skills:
			if (
				skill_id.contains("heal")
				or skill_id.contains("siphon")
				or skill_id.contains("restore")
			):
				if monster.can_use_skill(skill_id):
					return {
						"action": Enums.BattleAction.SKILL, "target": lowest_ally, "skill": skill_id
					}
	...
	for skill_id in monster.known_skills:
		if skill_id.contains("buff") or skill_id.contains("shield") or skill_id.contains("protect"):
			if monster.can_use_skill(skill_id):
				var buff_target: CharacterBase = (
					allies[randi() % allies.size()] if not allies.is_empty() else monster
				)
				return {
					"action": Enums.BattleAction.SKILL, "target": buff_target, "skill": skill_id
				}
...
func _try_use_skill(monster: Monster, targets: Array[CharacterBase]) -> Dictionary:
	...
	return {"action": Enums.BattleAction.SKILL, "target": target, "skill": selected_skill}
